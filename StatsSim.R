...
################# STATISTIC TESTS (2-TAILED) SIMULATION ################# 
...
frequentist_test <- function (visit_Start,visit_End,mean_Start,mean_End,var_Start,var_End,alpha,confidence,assessed_uplift){
  ...
  "pvalfreq <- (1 - pt(abs(t), df)) WE CANNOT USE ABS(t) BECAUSE THE DISTRIBUTION IS NOT NECESSARILY SYMMETRICAL
...
bayesian_test <- function(conv_Start,conv_End,fail_Start,fail_End,confidence,assessed_uplift){
  ...
  if((startbestthanend>=confidence)||(endbestthanstart<=1-confidence)){
    #print("Start")
    output <- data.frame(confidence,startbestthanend,-1,assessed_uplift)
    names(output) <- c("confidence","parameters","assessed_output","assessed_uplift")
  }else if((endbestthanstart>=confidence)||(startbestthanend<=1-confidence)){
    #print("End")
    output <- data.frame(confidence,endbestthanstart,1,assessed_uplift)
    names(output) <- c("confidence","parameters","assessed_output","assessed_uplift")
  }else{
    #print("No")
    output <- data.frame(confidence,max(startbestthanend,endbestthanstart),0,assessed_uplift)
    names(output) <- c("confidence","parameters","assessed_output","assessed_uplift")
  }
...
######  BOUNDS TEST  ######
#(visit_Start,visit_End,mean_Start,mean_End,var_Start,var_End,N_week,alpha,confidence,assessed_uplift,Bounds = Bounds,esti_obs,N_week_max)

bounds_test <- function (visit_Start,visit_End,mean_Start,mean_End,var_Start,var_End,N_week,alpha,confidence,assessed_uplift,Bounds,esti_obs,N_week_max) {
...
freq_bounds_test <- function (visit_Start,visit_End,mean_Start,mean_End,var_Start,var_End,N_week,alpha,confidence,assessed_uplift,Bounds,esti_obs,N_week_max) {
...
permutation_test <- function(Start_sample,End_sample,alpha,assessed_uplift) {
  ...
  "pval_perm <- mean(abs(delta_mean)<=perm_diff)
  ...
  if (pval_perm_sup<=alpha){
    ...
    assessed_output <- 0
    pval_perm <- min(pval_perm_inf,pval_perm_sup)
  }
...
bootstrap_test <- function(Start_sample,End_sample,alpha,assessed_uplift) {
  ...
  "
  "pval_boot <- mean(abs(delta_mean)<=boot_diff)
